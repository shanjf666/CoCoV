...
def upload_model_to_huggingface(hf_path):
    ...
    try:
        ...
        if "Transfer-Encoding" in str(e) or "NotImplemented" in str(e):
            print(f"方法1失败，原因: {e}")
            print("尝试方法2: 单文件逐个上传...")
        else:
            raise e
    ...
    try:
        for root, dirs, files in os.walk(hf_path):
            for file in files:
                ...
                # 计算相对路径作为repo中的路径
                relative_path = os.path.relpath(local_path, hf_path)
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt == max_retries - 1:
                            raise file_e
                        print(f"重试 {attempt + 1}/{max_retries}: {file_e}")
                        time.sleep(2)  # 等待2秒后重试
        
        print("单文件上传完成!")
    except Exception as e:
        print(f"方法2也失败了: {e}")
        print("请尝试手动上传或使用git-lfs")
        raise e
...
def convert_fsdp_checkpoints_to_hfmodels():
    ...
    try:
        ...
        print(f"Saving processor to {hf_path}")
        processor.save_pretrained(hf_path)
    except Exception as e:
        print(f"Note: No processor found or could not save processor: {e}")
