...
class TaskRunner:
    def run(self, config):
        ...
        elif reward_manager_name == "truelabel_ttrl":
        ...
        if reward_manager_name in ["ttrl", "truelabel_ttrl"]:
            val_reward_kwargs = dict(reward_kwargs)  # Copy training parameters
            val_reward_kwargs["mode"] = "eval"       # Override to eval mode
            val_reward_fn = reward_manager_cls(
                tokenizer=tokenizer, num_examine=1, compute_score=compute_score, reward_fn_key=config.data.reward_fn_key, eval_n_samples=config.actor_rollout_ref.rollout.val_kwargs.n, **val_reward_kwargs
            )
        else:
            val_reward_fn = reward_manager_cls(
