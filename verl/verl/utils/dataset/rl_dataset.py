...
class RLHFDataset(Dataset):
        ...
        if self.filter_overlong_prompts:
            ...
            def _doc_len(doc) -> int:
                ...
                try:
                    ...
                    # Robust fallback if structure is unexpected
                    text = "\n".join([m.get("content", "") for m in messages[:2]])
                return len(tokenizer(text, add_special_tokens=False)["input_ids"])

            self.dataframe = self.dataframe.filter(
                lambda doc: _doc_len(doc) <= self.max_prompt_length,
    ...
    def __getitem__(self, item):
        ...
        # Only use processor when multimodal data is actually present
    ...
    @staticmethod
    def _format_messages_to_text(messages: list, add_generation_prompt: bool = True) -> str:
        """Fallback formatter when tokenizer.chat_template is not available.

        Produces a simple readable prompt like:
        "System: ...\nUser: ...\nAssistant: ...\nAssistant: " (if add_generation_prompt)
        """
        role_prefix = {"system": "System", "user": "User", "assistant": "Assistant", "tool": "Tool"}

        def extract_text(content) -> str:
            # content may be a string or a list of typed segments
                ...
                for seg in content:
                    if isinstance(seg, dict):
                        if seg.get("type") == "text":
                            ...
                            # ignore non-text parts in fallback
                            continue
                return "".join(texts)
            return str(content)
        ...
        lines = []
        for msg in messages:
            role = role_prefix.get(msg.get("role", "user"), msg.get("role", "user").title())
